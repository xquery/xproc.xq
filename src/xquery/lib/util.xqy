...
{
     hof:apply-update-in(xdmp:database(),
     function(){
...
declare function u:xquery(
 ...
 return
      if (string-length($query) eq 0)
        ...
        else $context/xdmp:value( normalize-space($query) )
};
...
(: -------------------------------------------------------------------------- :)
declare function u:get-option($option-name as xs:string,$options as element(xproc:options)?,$primary){
(: -------------------------------------------------------------------------- :)
    $options/p:with-option[@name eq $option-name]/@select/data()
...
let $info := $const:error//err:error[@code=substring-after($error,':')]
    return
        ...
        error(fn:QName('http://www.w3.org/ns/xproc-error',$error),concat($error,": XProc Step Error - ",$string," ",$info/text(),'&#10;'))
};
...
:)
 ...
 (: ---------------------------------------------------------------- :)
 declare function u:enum-namespaces($pipeline) as element(namespace){
 (: ---------------------------------------------------------------- :)
...
return
        if ($ns eq 'xml' or $ns eq '')
        then ()
        else <ns prefix="{$ns}">{try{($element//* ! namespace-uri-for-prefix($ns,.))[1]}catch($e){ $e/error:description, $e/error:value, " module: ",
    $e/error:module, "(", $e/error:line-number, ",", $e/error:column-number, ")"}}</ns>
        }</namespace>
};


declare function u:get-step-functions(
    $filter as xs:string
)
{
    let $functions := u:get-step-functions()
    for $f in $functions
    return if(string(function-name($f)) eq $filter)
    then $f
    else ()
};
...
declare function u:get-step-functions()
{
    ...
    return $f[xdmp:annotation(.,xs:QName('xproc:step'))]
};

declare function u:validate($source)
{
    validate lax {$source}
};